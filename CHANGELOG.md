...
## v0.6.0-alpha.1 — Belief-Based Connected-Path Alpha

Major architectural change from the v0.5 follower experiments.

Highlights:

- Added asynchronous Follow-run recording with raw decision frames, events, steps, settings, calibration, and timeline data.
- Added read-only replay viewer and deterministic rerun support.
- Added persistent world-space path belief/state estimation.
- Added connected-path look-ahead planning and predicted continuation.
- Added exact-SVG ground-truth simulator with virtual camera motion.
- Added layered simulator DXF/run export and diagnostics.
- Added measured camera-realism model from recorded A–G test frames.
- Added timestamped observation-delay simulation.
- Added configured machine velocity/acceleration dynamics simulation.
- Added first real-machine belief follower with bounded safety gates.
- Seeded first belief tangent from selected Start dir to prevent false 180-degree startup innovation.
- Separated forward path progress from tuned lateral cross-track correction.
- Fixed connected-component association so tiny nearby noise does not hide a valid profile component.
- Changed physical execution from sequential X-then-Y jogs to one coordinated LinuxCNC X/Y G1 per bounded step.
- Added live planner overlay to the physical camera preview.
- Cleaned simulator UI and added direct simulator launcher.

Known alpha limitation: sharp connected-path vertices can still be turned slightly early. The architecture is intentionally still step-and-settle; planner-recommended velocity is logged but does not yet drive continuous velocity-jog motion.

---

# Legacy v0.5 Development Notes

## v0.5.3.10 - No Silent Corner Pause / Corner Fallback

This maintenance pass fixes the v0.5.3.9 corner-state dead end where Corner Pause could see a valid line/corner and then produce no `MOVE_PLAN`, no `STEP_REFUSED`, and no useful timeline explanation.

Changes:
- Fixed duplicate timeline keyword fields when Corner Assist and line-detection fields both included corner-intersection coordinates. This could interrupt the follow step before a planned/refused row was written, which looked like a silent pause.
- Corner Assist failures now fall back to ordinary bounded follow when the base line detection is still usable, instead of pausing without motion. The fallback is logged as `CORNER_ASSIST_FALLBACK` and the following `MOVE_PLAN` carries `corner_action=fallback_normal`.
- Corner Pause with Corner Assist disabled now writes an explicit `STEP_REFUSED` row with `corner_action=pause` and `corner_reason=assist_disabled`.
- Added explicit `STEP_REFUSED` logging for no outgoing heading, no latched line direction, and Max Turn stops after a successful detection.
- Preserves v0.5.3.9 Corner Commit behavior when a real corner is accepted: commit to the detected vertex, bypass delayed-position shortening, and latch the outgoing leg.

Unchanged:
- Delayed-position math for ordinary follow
- Virtual-target behavior for ordinary follow
- Fresh-frame retry
- Garbage-frame retry
- Curve-aware sanity
- One-shot corner re-arm behavior
- DXF export
- LinuxCNC command style
- Z / torch / plasma behavior

Validation:
- `python3 -m py_compile fabscan/app.py fabscan/camera_calibration.py`

## v0.5.3.9 - Corner Commit

- Changes Corner Assist from a cautious nudge/defer behavior into an explicit corner-vs-radius decision. If the recent incoming heading is already rotating, FabScan classifies the candidate as radius/profile geometry, suppresses Corner Assist, and continues ordinary bounded follow.
- If the incoming heading is stable and the fitted intersection is inside the configured Lookahead window, FabScan commits to the detected vertex and drives the camera/crosshair directly to that intersection. The committed vertex move is allowed up to the Lookahead distance and bypasses delayed-position compensation plus the normal Follow Step + Max Correct limiter so it does not get shortened into a rounded corner.
- Keeps the v0.5.3.7 one-shot/re-arm behavior after a committed corner: repeat corner assists stay suppressed until the corner candidate clears for clean frames.
- Adds explicit timeline logging for the corner crosshair/intersection pixel position, corner offset, machine-space corner move, outgoing heading, corner classification, and incoming-heading stability metrics.
- This is still X/Y-only bounded position motion. No velocity-jog loop, Z, torch, plasma, DXF export, or LinuxCNC command-style change was intentionally added.
- About/title updated to `FabScan v0.5.3.9 - Corner Commit`.

## v0.5.3.8 - Corner Assist Engage Distance

- Splits Corner Assist detection lookahead from Corner Assist engagement distance. FabScan can still detect and log an upcoming corner inside the Lookahead window, but it now defers to ordinary bounded follow steps until the calculated vertex is close enough to reach in one bounded assist move.
- Adds `corner_engage_limit` to the timeline log so corner distance, lookahead limit, and actual assist-engage distance can be compared directly.
- Keeps the v0.5.3.7 one-shot/re-arm behavior: after a successful assist, Corner Assist remains consumed until the corner candidate clears for clean frames.
- Intended to reduce early turns/rounded corners caused by assist firing when the vertex is visible but still several steps away.
- About/title updated to `FabScan v0.5.3.8 - Corner Assist Engage Distance`.

## v0.5.3.7 - Corner Assist Re-Arm Fix

- Tightens the v0.5.3.6 one-shot Corner Assist state machine.
- After a successful Corner Assist move, repeat assist is suppressed until `corner_candidate` clears for consecutive clean frames.
- Removes the prior behavior where a short ordinary-step cooldown could re-arm Corner Assist while the camera still saw the same corner/radius/tab clutter.
- Suppression now happens before attempting another corner-intersection solve, so consumed corners cannot repeatedly fail with new `no safe intersection` refusals.
- Timeline logging adds corner clear-frame counts and logs `CORNER_ASSIST_CLEAR_FRAME` events.
- No changes to delayed-position math, virtual-target math, fresh-frame retry, garbage-frame retry, curve-aware sanity, DXF export, LinuxCNC command style, or Z/torch/plasma behavior.

## v0.5.3.6 - One-Shot Corner Assist

- Makes Corner Assist a one-shot event per detected corner/ambiguous zone instead of a repeatedly re-entered mode.
- After a successful corner-assist move, FabScan suppresses further corner assists until either the detected corner clears for a clean frame or a short ordinary-follow-step cooldown expires.
- Adds timeline events/fields for corner-assist consumed, suppressed, cooldown-step, and cleared states. This is intended to fix the “stuck on the corner” behavior where repeated tiny assist moves kept firing on the same top-tab/corner area.
- Keeps v0.5.3.5 Corner Assist defer behavior: a corner seen outside the lookahead window still falls through to a normal bounded follow step instead of halting.
- No intentional changes to delayed-position math, virtual-target math, fresh-frame retry, curve-aware sanity, DXF export, LinuxCNC command style, or Z/torch/plasma behavior.
- About/title updated to `FabScan v0.5.3.6 - One-Shot Corner Assist`.

## v0.5.3.5 - Corner Assist Defer / Hough Cluster Logging

- Adds a graceful "not yet" path for Corner Assist. If a valid corner is detected but the fitted intersection is outside the configured Lookahead distance, Follow N logs `CORNER_DEFERRED` and continues with the normal bounded follow step instead of halting the run.
- Keeps hard stops for unsafe Corner Assist failures such as no safe intersection, bad calibration, invalid move, no outgoing heading, or already-at-corner cases.
- Adds Hough/corner cluster diagnostics to the timeline log: segment count, cluster count, top three cluster angles/lengths, and corner reject reason. This should help diagnose scenes where three nearby edges are visible and the standard corner detector does not flag a clean two-line corner.
- About/title updated to `FabScan v0.5.3.5 - Corner Assist Defer / Hough Cluster Logging`.

## v0.5.3.4 - Curve-Aware Sanity / Garbage-Frame Retry

- Adds a curve/profile-aware sanity path for normal Follow Step / Follow N. Strong, high-quality detections can pass through a larger legitimate heading change while the existing Max turn, confidence, progress lock, correction limits, and LinuxCNC readiness checks still apply.
- This is intended for part-like profiles and radii where the old hard Sanity° wall could stop a valid trace at real direction changes.
- Expands the v0.5.3.3 fresh-frame retry path to handle obvious garbage detections, not only complete `not_found` frames. If a frame suddenly collapses to a tiny span, sparse point count, or very low confidence compared with the recent accepted feature, FabScan waits for a fresh frame and retries before the normal refusal logic runs.
- Timeline CSV logs now include sanity action/reason/limit/change fields plus a garbage retry reason so profile-turn allowances and bad-frame retries can be diagnosed from the log.
- No delayed-position math, virtual-target math, DXF export, corner assist, Z/torch/plasma behavior, or LinuxCNC command style was intentionally changed.
- About/title updated to `FabScan v0.5.3.4 - Curve-Aware Sanity / Garbage-Frame Retry`.
